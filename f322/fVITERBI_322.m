%============================================================================
%  fVITERBI_322.m 
%  (3,2,2)Forward Label Viterbi Decoder.
%
...
for t=1:L-1,
   ...
   if t==1,
      add0(1,1)=acc0(1,1)+hd(1,1); tr_hist(st0+1,t+1)=0;st_hist(st0+1,t+1)=0;
      add1(1,1)=acc0(1,1)+hd(1,2); tr_hist(st1+1,t+1)=0;st_hist(st1+1,t+1)=0;
      add2(1,1)=acc0(1,1)+hd(1,3); tr_hist(st2+1,t+1)=0;st_hist(st2+1,t+1)=0;
      add3(1,1)=acc0(1,1)+hd(1,4); tr_hist(st3+1,t+1)=0;st_hist(st3+1,t+1)=0
      
      acc0(1,1)=add0(1,1); pathmem(st0+1,(k*(t-1)+k)+1:(k*t)+k)=D2B(0,k);
      acc1(1,1)=add1(1,1); pathmem(st1+1,(k*(t-1)+k)+1:(k*t)+k)=D2B(1,k);
      acc2(1,1)=add2(1,1); pathmem(st2+1,(k*(t-1)+k)+1:(k*t)+k)=D2B(2,k);
      acc3(1,1)=add3(1,1); pathmem(st3+1,(k*(t-1)+k)+1:(k*t)+k)=D2B(3,k);
      acc=[acc0 acc1 acc2 acc3 acc4 acc5 acc6 acc7];
   elseif t==2,
      [acc,pathmem,st_hist,tr_hist]=fACSU_322(t,k,L,hd,acc,pathmem,st_hist,tr_hist);
   end;
end;
...
while t <= N+m,
   ...
   if t < (T+1),
      ...
      [acc,pathmem,st_hist,tr_hist]=fACSU_322(stage,k,L,hd,acc,pathmem,st_hist,tr_hist);
   else
      % Perform traceback read & output one decoded stage
      [Ux_, MLDpath]= fTRACEBACK_322(pathmem,acc,st_hist,Ux_,MLDpath,k,m,M,N,T,stage);
...
if plottrellis==1,
   fTRELLIS_322(decMessage,MLDpath,tr_hist,n,k,M,N);
end;
